...
<body>
  ...
  <form id="loginOptionsContainer" method="post" action="" style="z-index: 70">
    <div id="formGrid">
      <h6>
        <?php if (!isset($_SESSION['user'])) echo "Not logged in";
        else echo "Logged in as <em> " . $_SESSION['user'] . " </em>" ?>
      </h6>
      <?php if (!isset($_SESSION['user'])) { ?>
        <input class="loginField" type="text" name="username" id="loginId" placeholder="username" required />
        <input class="loginField" name="password" id="loginPass" placeholder="password" required type="password" />
        ...
        <div id="signup-hint">
          ...
          <button class="loginOptions" id="signUp" type="submit" name="register">
            Register
          </button>
        </div>
      <?php } else { ?>
        <button class="loginOptions" id="logout" type="submit" name="logout">
          Logout
        </button>
      <?php } ?>
    </div>
  ...
  <div id="mainContainerContent" style="z-index: 60">
    ...
    <div id="main">
      <div id="gridMain">
        <input type="text" id="valInput" placeholder="type something" autocomplete="off" />
        <button class="submit" id="ajouter" onclick="entrerVal(document.getElementById('valInput').value)">
          ADD
        </button>
        <button onclick="suppr()" class="submit" id="tSuppr">CLEAR</button>
        <form action="" method="post" id="save">
          <button class="submit" type="submit" name="save" onclick="saveLists()">
            SAVE
          </button>
          <input type="text" name="lists" id="hiddenInput" style="display: none" />
        </form>
      </div>
  ...
  <h4 onclick="window.open('https://dcjetc.com/meriadec-legal/portfolio', '_blank')">
